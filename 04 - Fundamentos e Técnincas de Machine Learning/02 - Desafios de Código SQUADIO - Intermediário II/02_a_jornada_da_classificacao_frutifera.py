...
# características únicas que as diferenciam. Seu objetivo é criar um modelo de machine learning capaz de
# classificar frutas com base em três características: peso, textura (suave ou áspera) e cor (vermelha, 
# laranja ou amarela). Cada tipo de fruta tem limites específicos para essas características.
...
def prever_fruta(peso, textura, cor):
  if textura == "rough":
    if cor == "red" and peso >= 130:
      return "A fruta é uma maçã!"
    elif cor == "yellow" and peso >= 150:
      return "A fruta é uma banana!"
  elif textura == "smooth":
    if cor == "red" and peso >= 150:
      return "A fruta é um morango!"
    elif cor == "orange" and peso >= 120:
